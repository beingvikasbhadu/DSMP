...
public class URLDAOWrapper {
	...
	public URLBean saveEntity(URLBean bean) throws Exception {
		/// if this url is already shorten they we don't have to shorten it
		   URLEntity search_url=urlDAO.findByUrl(bean.getUrl());
		   if(search_url!=null)
			   return entityToBean(search_url);
	...
	public URLBean updateEntityByUrl(URLBean bean) throws Exception {
		...
		URLEntity entity = urlDAO.findByShortCode(bean.getShortCode());
		System.out.println("ENTITY:"+entity);
		if (entity != null) {
